...
class DocketUserAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        qs = super(DocketUserAdmin, self).get_queryset(request)
            ...
            if db_field.name == 'groups':
                if group.name == 'admin':
                    ...
                    kwargs["queryset"] = AuthGroups.objects.filter(
                        name__in=groups)
        return super(DocketUserAdmin, self).formfield_for_manytomany(
            db_field, request, **kwargs)

    def get_actions(self, request):
        actions = super(DocketUserAdmin, self).get_actions(request)
        if not request.user.is_superuser:
...
admin.site.register(DocketUser)#, DocketUserAdmin)